...
public class ComputerCraft {
    public static void init() {
        ComputerCraftAPI.registerPeripheralProvider(new IPeripheralProvider() {
            @Nullable
            @Override
            public IPeripheral getPeripheral(@Nonnull World world, int x, int y, int z, int enumFacing) {
                TileRailBase rail = cam72cam.mod.world.World.get(world).getBlockEntity(new Vec3i(x, y, z), TileRailBase.class);
                if (rail != null) {
                    if (rail.getAugment() == Augment.DETECTOR) {
                        return new DetectorPeripheral(world, new Vec3i(x, y, z));
                    }
                    if (rail.getAugment() == Augment.LOCO_CONTROL) {
                        return new LocoControlPeripheral(world, new Vec3i(x, y, z));
                    }
                }
                return null;
            }
    ...
    private static abstract class BasePeripheral implements IPeripheral {
        ...
        public BasePeripheral(World world, Vec3i blockPos, LinkedHashMap<String, APICall> methods) {
            this.world = world;
            this.pos = blockPos;
    ...
    private static class DetectorPeripheral extends BasePeripheral {
        ...
        public DetectorPeripheral(World world, Vec3i blockPos) {
    ...
    private static class LocoControlPeripheral extends BasePeripheral {
        ...
        public LocoControlPeripheral(World world, Vec3i blockPos) {
            super(world, blockPos, methods);
